...
# добавляем гидрологические районы
# гидрологические районы
# 1 - Каспийский
# 2 - Черноморский
# 3 - Азовский
# 4 - Балтийский
# 5 - Баренцево-Беломорский
# 6 - Карский
# 7 - Восточно-Сибирский
# 8 - Тихоокеанский


# добавляем номер гидробиологического района
...
# выправляем названия водных объектов и пунктов отбора проб
...
# СПРАВОЧНИК таблица с водными объектами ------
waterBodiesTable <- function(wTable) {
  # структура таблицы
  drtable = 'DROP TABLE IF EXISTS water_bodies;'
  ...
  dltable = 'DELETE FROM water_bodies;'
  
  # dbExecute(connection, drtable)
...
# импорт гидробиологических пунктов
...
# справочная таблица с гидробиологическими пунктами
pointsTable <- function(wTable) {
# структура таблицы
drtable = 'DROP TABLE IF EXISTS hydrobiological_stantions;'
...
dltable = 'DELETE FROM hydrobiological_stantions;'
...
# dbExecute(connection, dltable)
  ...
  drview = 'DROP VIEW IF EXISTS stations;'
...
# функция возвращающая ID вида, уже внесенного в базу
...
# функция добавляющая по заданному названию сведения о виде из GBIF Taxonomy Backbone
...
speciesTable <- function(wTable) {
  ...
  # процедуру начинаем с удаления и создания таблицы
  drsp = 'DROP TABLE IF EXISTS species;'
  crsp = paste0('CREATE TABLE IF NOT EXISTS species (ids integer PRIMARY KEY AUTOINCREMENT,',
              # 'sp_verbatim varchar UNIQUE,', 
              'sp_verbatim varchar,',
              'scientific_name varchar,',
              'backbone_key integer, rank varchar, status varchar, match varchar,',
              'kingdom varchar, phylum  varchar, class_ varchar, order_ varchar,',
              'family_ varchar, genus varchar);')
  dlsp = 'DELETE FROM species;'

  dbExecute(connection, drsp)
  dbExecute(connection, crsp)
  # dbExecute(connection, dlsp)

  # таблица с сапробнотью
  drsapr = 'DROP TABLE IF EXISTS saprobity;'
  ...
  dlsapr = 'DELETE FROM saprobity;'

  dbExecute(connection, drsapr)
  dbExecute(connection, crsapr)
  # dbExecute(connection, dlsapr)
  ...
  drview = 'DROP VIEW IF EXISTS taxons;'
  ...
  drview = 'DROP VIEW IF EXISTS sp_saprobity;'
...
rawTables <- function() {
  # ТАБЛИЦЫ СО СБОРАМИ
  drtable = 'DROP TABLE IF EXISTS samples;'
  ...
  drtable = 'DROP TABLE IF EXISTS occurrences;'
  ...
  dltable = 'DELETE FROM occurrences;'
  ...
  drtetable = 'DROP TABLE IF EXISTS samples_calculation;'
  createtable = paste0('CREATE TABLE samples_calculation (ids integer PRIMARY KEY AUTOINCREMENT,',
                       ' sample_id int REFERENCES samples(ids) ON UPDATE CASCADE ON DELETE CASCADE,',  
                       ' shannon decimal(8,7), pielou decimal(8,7), ind_species_count int,',
                       ' ind_count_ratio decimal(3,1), ind_biomass_ratio decimal(3,1));')
  dbExecute(connection, drtetable)
...
createViews <- function() {
  ...
  crview = paste0('CREATE VIEW saprobity_1 AS SELECT ids, occurrences.sample_id,',
                  ...
                  'ind_value,count_m3,biomass_m3, biomass_sum_m3, amount_sum_m3,',
                  'round(count_m3 / amount_sum_m3 * 100,1) count_m3_percents,',
                  'CASE WHEN round(count_m3 / amount_sum_m3 * 100,1) BETWEEN 0 AND 1 THEN 1 ',
                  'WHEN round(count_m3 / amount_sum_m3 * 100,1) BETWEEN 1 AND 3 THEN 2 ',
                  'WHEN round(count_m3 / amount_sum_m3 * 100,1) BETWEEN 3 AND 10 THEN 3 ',
                  'WHEN round(count_m3 / amount_sum_m3 * 100,1) BETWEEN 10 AND 20 THEN 5 ',
                  'WHEN round(count_m3 / amount_sum_m3 * 100,1) BETWEEN 20 AND 40 THEN 7 ',
                  'WHEN round(count_m3 / amount_sum_m3 * 100,1) BETWEEN 40 AND 100 THEN 9 ',
  ...
  crview = paste0('CREATE VIEW species_records_all AS SELECT total_dump.sample_id,',
                  ...
                  'round(count_m3 / amount_sum_m3 * 100,1) count_m3_percent,',
                  'round(biomass_m3 / biomass_sum_m3 * 100,1) biomass_m3_percent ',
                  'FROM total_dump JOIN samples_sum ON ',
                  'samples_sum.sample_id = total_dump.sample_id;')
  dbExecute(connection, drview)
  dbExecute(connection, crview)
  
  drview = 'DROP VIEW IF EXISTS saprobity_2;'
  crview = paste0('CREATE VIEW saprobity_2 AS SELECT sample_id, sum(score) score_sum,',
                  'sum(count_m3_percents) percent_sum,round(sum(score * ind_value) / sum(score),2) saprobity ',
                  'FROM saprobity_1 GROUP BY sample_id ORDER BY sample_id;')
  dbExecute(connection, drview)
  dbExecute(connection, crview)
  
  drview = 'DROP VIEW IF EXISTS sample_summary;'
  crview = paste0('CREATE VIEW sample_summary AS SELECT samples_sum.sample_id, samples_sum.sample_code,',
                  ...
                  'ind_species_count,ind_count_ratio,ind_biomass_ratio,amount_sum,',
                  'biomass_sum, amount_sum_m3, biomass_sum_m3, shannon,',
                  'pielou,saprobity FROM samples_sum LEFT JOIN ',
...
stationID <- function(code,v) {
  ...
  # сначала проверяем, сколько станций с таким кодом
  if (v==1) {
...
# функция для удаления всех уже внесенных записей
...
sample2base <- function(sampletable,filial) { 
  ...
  # write.csv(samples, paste0('raw/samples_check_',filial,'.csv'))
  ...
  # write.csv(samplesu, paste0('samples_',filial,'_',as.integer(Sys.time()),'.csv'))
  # cat('Список проб записан в CSV файл \n')
  
  if (nrow(samplesu) > 0) {
    cat(paste0('число проб - ',nrow(samplesu),'\n'))
  } else {
  ...
  for (i in 1:n) {
    ...
    sCode = sampleCode(filial,'Z',2021)
...
occur2base <- function(sampletable,filial) {
  ...
  # write.csv(occurs, '../source/samples_2021/occurs_temp.csv')
  ...
  for (i in 1:n) {
    ...
    sp[sp == 'копеподиты'] = 'Hexanauplia' # key 9421786  Copepoda - это подкласс, промежуточных таксонов в GBIF нет
    sp[sp == 'Copepoditi'] = 'Hexanauplia' # key 9421786
    sp[sp == 'Copepodita'] = 'Hexanauplia' # key 9421786
    sp[sp == 'Copepoda nauplii I-VI'] = 'Hexanauplia'
    sp[sp == 'Nauplii Copepoda'] = 'Hexanauplia'
    sp[sp == 'Nauplii copepoda'] = 'Hexanauplia'
    sp[sp == 'Nayplii Copepoda'] = 'Hexanauplia'
    sp[sp == 'Copepoda sp.'] = 'Hexanauplia'
    sp[sp == 'Copepoda'] = 'Hexanauplia'
    sp[sp == 'Kopepodit'] = 'Hexanauplia'
    ...
    group[group == 'Веслоногие'] = 'Hexanauplia' # класс
    ...
    # приводим численность на кубометр к нужной размерности - в единицах, а не тыс. экз., для 5ти филиалов
    fls = c('NN','DV','ZB','IR','MM')
    if (filial %in% fls) {
      # countm3 = countm3 / 1000
      # переводим тысячи в единицы экз. на кубометр
      if (!is.na(countm3)) {
        # cat(paste0('\n экз. на куб: ',countm3,'\n'))
        countm3 = as.numeric(countm3) * 1000   
      }
    }
...
importRawTable <- function(rawFile) {
  ...
  cat(paste0('импортирумый файл: ',rawFile,' филиал: ',filial,'\n'))
  ...
  for (i in 1:sheetcount) {
    ...
    sample2base(sampletable,filial) # вносим пробы в базу с очередного листа 
...
# число видов в пробе с индикаторными значениями
indSpCount <- function(sampleID) {
  select = paste0('SELECT count(DISTINCT occurrences.backbone_key) FROM occurrences ',
           'JOIN species ON species.backbone_key = occurrences.backbone_key ',
           'JOIN saprobity ON saprobity.species_id = species.ids ',
           'WHERE sample_id = ',sampleID,' AND ind_value IS NOT NULL;')
  result = dbGetQuery(connection, select)
  return(result[1])
}

# доля индикаторныз видов по биомассе и численности
indSpRatio <- function(sampleID) {
  cat(paste0('доля обилия индикаторных видов в пробе: ',sampleID,'\n'))
  select = paste0('SELECT sum(count_m3) FROM ',
                  '(SELECT DISTINCT count_m3, scientific_name FROM occurrences  ',
                  'JOIN species ON species.backbone_key = occurrences.backbone_key ',
                  'JOIN saprobity ON saprobity.species_id = species.ids ',
                  'WHERE sample_id = ',sampleID,' AND ind_value IS NOT NULL);')
  indCount = dbGetQuery(connection, select)

  select = paste0('SELECT sum(count_m3) FROM occurrences ',
                  'WHERE sample_id = ',sampleID,';')
  totalCount = dbGetQuery(connection, select)
  
  select = paste0('SELECT sum(biomass_m3) FROM ',
                  '(SELECT DISTINCT biomass_m3, scientific_name FROM occurrences  ',
                  'JOIN species ON species.backbone_key = occurrences.backbone_key ',
                  'JOIN saprobity ON saprobity.species_id = species.ids ',
                  'WHERE sample_id = ',sampleID,' AND ind_value IS NOT NULL);')
  indBiomass = dbGetQuery(connection, select)

  select = paste0('SELECT sum(biomass_m3) FROM occurrences ',
                  'WHERE sample_id = ',sampleID,';')
  totalBiomass = dbGetQuery(connection, select)
  result = c(round(indCount/totalCount*100,1),round(indBiomass/totalBiomass*100,1))
  cat('Result ')
  cat(paste(result))
  cat('\n')
  return(result)
...
indices <- function() {
  ...
  for (i in 1:n) {
    ...
    indcount = indSpCount(id)
    indratio = indSpRatio(id)
    countRatio   = indratio[1]
    biomassRatio = indratio[2]
    if (is.na(countRatio)) countRatio = 'null'
    if (is.na(biomassRatio)) biomassRatio = 'null'
    
    cat(paste0('проба: ',id,' Шеннон: ',sh,'\tПилау: ',pl,'\tчисло индикаторных видов : ',indcount,'\n'))
    insert = paste0('INSERT INTO samples_calculation (sample_id, shannon, pielou, ',
                    ' ind_species_count, ind_count_ratio, ind_biomass_ratio) ',
                    ' VALUES (',id,',',sh,',',pl,',',indcount,',',countRatio,',',biomassRatio,');')
    cat(insert,'\n')
    dbExecute(connection, insert)
  }
  # выгрузка сводной таблицы с пробами
  selectToCSV('sample_summary',1)
